...
#TODOS OS LIVROS DE BIOGRAFIA
SELECT * FROM LIVROS WHERE CATEGORIA = 'BIOGRAFIA';
#TODOS OS ROMANCES QUE CUSTAM MENOS DE 48 REAIS
SELECT * FROM LIVROS WHERE CATEGORIA = 'ROMANCE' AND PRECO < 48;
#POESIAS QUE NÃO SÃO NEM DE LUIS VAZ DE CAMÕES NEM DE GABRIEL PEDROSA
SELECT * FROM LIVROS WHERE CATEGORIA = 'POESIA' AND NOT (AUTORIA IN ('LUÍS VAZ DE CAMÕES', 'GABRIEL PEDROSA'));

SELECT * FROM VENDAS;

#SELECIONAR TODAS AS VENDAS DE LIVROS DISTINTOS FEITAS PELO VENDEDOR 1
SELECT DISTINCT * FROM VENDAS WHERE ID_VENDEDOR = 1
ORDER BY ID_LIVRO; 

SET FOREIGN_KEY_CHECKS = 1;

#ALTERANDO O VALOR DE TODOS OS LIVROS PARA QUE FIQUE COM 10% DE DESCONTO COM CHECKS ATIVO

UPDATE LIVROS  SET PRECO = PRECO*0.9;
...
#QUANTITIVO DE VENDAS POR VENDEDOR

SELECT A.ID_VENDEDOR, B.NOME_VENDEDOR, SUM(A.QTD_VENDIDA)
FROM VENDAS AS A, VENDEDORES AS B
...
SELECT A.ID_VENDEDOR, B.NOME_VENDEDOR, SUM(A.QTD_VENDIDA)
FROM VENDAS AS A INNER JOIN VENDEDORES AS B
ON A.ID_VENDEDOR = B.ID_VENDEDOR
GROUP BY ID_VENDEDOR;

#VERIFICAR SE TODOS OS LIVROS DA BASE DE DADOS FORAM VENDIDOS

SELECT A.NOME_LIVRO, SUM(B.QTD_VENDIDA)
FROM LIVROS AS A LEFT JOIN VENDAS AS B
